...
/*p_s_vector vector_alloc(size_t n) {
    p_s_vector v2Double = malloc(sizeof(p_s_vector));
...
p_s_vector vector_alloc(size_t n){
    p_s_vector tab = (p_s_vector) malloc(sizeof(s_vector));
    if(tab != NULL){
        tab->size = n;
...
void vector_insert(p_s_vector p_vector, size_t i, void* v){
    if(i<=p_vector->size) {
        ...
        for (int j = p_vector->size; j > i; j--) {
            vector_set(p_vector,j,get(p_vector,j-1));
        }
...
p_s_vector vector_v2_copy_vector(p_s_vector p_vector) {
    p_s_vector v2Double = vector_alloc(p_vector->size);
    for (int i = 0; i < p_vector->size; ++i) {
        vector_set(v2Double,i,p_vector->p[i]);
    }
