...
int test_index_change_not_exists() {
    p_s_vector p_vector = vector_alloc(1);
    p_s_vector p_vector_old = vector_v2_copy_vector(p_vector);
    vector_set(p_vector, 2, 2.0);
    if(vector_v2_comp_vectors(p_vector_old,p_vector)) {
	vector_free(p_vector);
	vector_free(p_vector_old);
	return 1;
    }
    vector_free(p_vector);
    vector_free(p_vector_old);
...
int test_set_index_existant(){
    p_s_vector v2Double = vector_alloc(5);
	vector_set(v2Double, 3, 6); //test

	if (get(v2Double,3) == 6){
		vector_free(v2Double);
		return 1; //bon
	} else {
		vector_free(v2Double);
		return 0; //problème
...
int test_pop_back(){
    p_s_vector v2Double = vector_alloc(3);

	vector_pop_back(v2Double); //test
...
int test_init_tableau(){
    size_t size = 5;
    p_s_vector p_vector = vector_alloc(size);
...
int test_add_pos3() {
    p_s_vector p_vector = vector_alloc(5);
    vector_insert(p_vector,3,40);
...
int test_add_pos10() {
    p_s_vector p_vector = vector_alloc(5);
    p_s_vector p_vector_copy = vector_v2_copy_vector(p_vector);
    vector_insert(p_vector,10,40);
    if(vector_v2_comp_vectors(p_vector, p_vector_copy)) {
	vector_free(p_vector);
	vector_free(p_vector_copy);
	return 1;
    }
    vector_free(p_vector);
    vector_free(p_vector_copy);
...
int test_del_pos3() {
    p_s_vector p_vector = vector_alloc(5);
    vector_insert(p_vector,4,50);
    vector_erase(p_vector,3);
