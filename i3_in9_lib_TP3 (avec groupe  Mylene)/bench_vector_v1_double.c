//appel par : ./bench_vector_v1_double test_type init_size n
...
void insert_erase_random(p_s_vector_v1_double p_vector, size_t n){
	...
	for(int i = 0; i < n; i++){
		...
		vector_v1_double_insert(p_vector,pos,element);
	}
	//Suppression de n elements aleatoires
	for(int i = 0; i < n; i++){
		pos = random_size_t(0, p_vector->size-1);
		vector_v1_double_erase(p_vector,pos);
	}
}
...
void insert_erase_tail(p_s_vector_v1_double p_vector, size_t n){
	double element;
	size_t pos = vector_v1_double_size(p_vector) - 1;
...
void read_write_random(p_s_vector_v1_double p_vector, size_t n){
	...
	for(int i = 0; i < n; i++){
		element = random_double(0,200);
		pos = random_size_t(0, p_vector->size-1);
		vector_v1_double_set(p_vector,pos,element);
	}
...
void read_write_sequential(p_s_vector_v1_double p_vector, size_t n){
	...
	for(int i = 0; i < n; i++){
		for(int j = 0; j < size ; j++){	
			element = random_double(0,200);
			vector_v1_double_set(p_vector,j,element);
		}
	}
...
int main(int argc, char* argv[]) {
    ...
    p_s_vector_v1_double v = vector_v1_double_alloc(size);

	for(int i=0; i<v->size; i++) {
	    vector_v1_double_set(v, i, random_double(0,1));
	}
