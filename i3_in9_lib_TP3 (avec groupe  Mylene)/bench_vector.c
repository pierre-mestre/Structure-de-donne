//appel par : ./bench_vector test_type init_size n
...
void insert_erase_tail(p_s_vector p_vector, size_t n){
	double element;
	size_t pos = vector_size(p_vector) - 1;
...
void read_write_random(p_s_vector p_vector, size_t n){
	...
	for(int i = 0; i < n; i++){
		element = random_double(0,200);
		pos = random_size_t(0, p_vector->size-1);
		vector_set(p_vector,pos,element);
	}
...
int main(int argc, char* argv[]) {
	if(argc < 4) {
		printf("Usage : ./bench_vector test_type init_size n\n");
    ...
    p_s_vector v = vector_alloc(size);

	for(int i=0; i<v->size; i++) {
	    vector_set(v, i, random_double(0,1));
	}
